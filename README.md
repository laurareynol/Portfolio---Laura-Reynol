...
Fundo escuro, painéis de "papel" (`.folha`) para texto, títulos grandes em carimbo entre colchetes (`.carimbo`, ex. `[ WORK . ]`) e vinho (`--vinho`) como única cor de destaque.

## Estrutura

- `index.html` — portfólio atual.
- `arquivo/index-antigo.htm` — versão anterior do site (estilo azul/laranja), mantida só como referência. Não está em uso.
